...
function doApproved(){
    ...
    if (isset($_POST['submit'])) {
        ...
        $status = $_POST['STATUS'];
        $datetime_approved = date('Y-m-d H:i:s');
        
        $sql = "UPDATE `tbljobregistration` SET `REMARKS`='{$remarks}', `PENDINGAPPLICATION`=0, `HVIEW`=0, `DATETIMEAPPROVED`='{$datetime_approved}', `STATUS`='{$status}' WHERE `REGISTRATIONID`='{$id}'";
        $mydb->setQuery($sql);
        $cur = $mydb->executeQuery();
		...
		if ($status == 'Hired') {
    // insert new record in tblemployees
    $sql = "INSERT INTO `tblemployees` (`EMPLOYEEID`, `F_FNAME`, `F_MNAME`, `F_LNAME`, `F_ADDRESS`, `SEX`, `F_CONTACTNO`, `F_EMAILADDRESS`, `AGE`, `WORKSTATS`, `DATEHIRED`, `POSITION`, `c_id`, `j_id`, `PAYMENT`) 
        SELECT `tblapplicants`.`APPLICANTID`, `FNAME`, `MNAME`, `LNAME`, `ADDRESS`, `SEX`, `CONTACTNO`, `EMAILADDRESS`, `AGE`, 'In progress', NOW(), `OCCUPATIONTITLE`, `tbljob`.`client_id`, `tbljobregistration`.`JOBID`, `tbljob`.`SALARIES`
        FROM `tblapplicants`
        INNER JOIN `tbljobregistration` ON `tblapplicants`.`APPLICANTID` = `tbljobregistration`.`APPLICANTID`
        INNER JOIN `tbljob` ON `tbljobregistration`.`JOBID` = `tbljob`.`JOBID`
