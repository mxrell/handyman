...
function doInsert($jobid=0,$fileid=0) {
	...
	global $mydb; 
			...
			$sql = "SELECT c.*, j.*, j.`OCCUPATIONTITLE` 
			FROM `tblcompany` c, `tbljob` j 
			...
			$jobreg->F_TITLE = $result->OCCUPATIONTITLE;
			$jobreg->STATUS = 'Pending'; 
			$jobreg->c_id =  $result->client_id;
			$jobreg->create();
...
function doRegister(){
	...
	if (isset($_POST['btnRegister'])) { 
    	...
    	$front_id = $_FILES['FRONT_ID']['name'];
    	$back_id = $_FILES['BACK_ID']['name'];

    	// move the uploaded files to a directory
    	move_uploaded_file($_FILES['FRONT_ID']['tmp_name'], "appl_id/" . $front_id);
    	move_uploaded_file($_FILES['BACK_ID']['tmp_name'], "appl_id/" . $back_id);

    	// insert the file names into the database
    	$applicant->FRONT_ID = $front_id;
    	$applicant->BACK_ID = $back_id;
}
